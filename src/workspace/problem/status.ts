...
import { DIAGNOSTIC_CONTENT_TEXT, DIAGNOSTIC_GLYPH, DIAGNOSTIC_ENTRY_LIST } from '../../store/state';
...
import { updateDiagnostic } from './diagnostic';
import { resetDecoration } from '../../editor/editor';
import { createCursorRange } from '../../editor/range';
...
const diagnosticContentText = DIAGNOSTIC_CONTENT_TEXT as D.Diagnostic.Intf.DiagnosticContentText;

const lineGlyph = DIAGNOSTIC_GLYPH as D.Diagnostic.Intf.LineGlyph;
...
const diagnosticInfo = (decorationState: D.Decoration.Intf.State) => (editor: vscode.TextEditor): void => {
    ...
    diagnosticContentText[diagnosticRenderSignature(stateBuffer) as D.Numeric.Key.Bin].forEach(renderDiagnosticText(editor.setDecorations));
};
