...
var multiCursorLineCountHex = 520 /* LC */;
...
var cursorOnlySelection = (editor2, previousKey) => {
  ...
  const option = {
    // options to be passed to decroationType
...
var contentTextFuncBuffered = (setDecorations, buffer, resBuffer, channel) => (renderOption, idx) => {
  optionBufferChannel[channel][0].renderOptions = renderOption;
  if (typeof renderOption.after.contentText !== "string") {
    const hexKey = renderOption.after.contentText;
    if (resBuffer[hexKey] === -1) {
      resBuffer[hexKey] = multilineFnHexLink[hexKey](
        vscode3.window.activeTextEditor
        // call function 
      );
    }
    decorationOptionBuffer.after = { ...renderOption.after };
    decorationOptionBuffer.after.contentText = String(resBuffer[hexKey]);
...
var optionBufferChannel = [
  [/* @__PURE__ */ Object.create({ ...decorationOptionBuffer })],
  [/* @__PURE__ */ Object.create({ ...decorationOptionBuffer })]
];
var multiLineOfStatus = {
  [multiLineLineCountHex]: -1,
  [multiLineChararcterHex]: -1
};
var multilineSelection = (editor2, previousKey) => {
  multiLine !== previousKey[0] && clearBufferOfhexkey(editor2.setDecorations, previousKey);
  multiLineOfStatus[multiLineLineCountHex] = -1;
  multiLineOfStatus[multiLineChararcterHex] = -1;
  optionBufferChannel[0][0] = {
    // anchor decorationOption 
    ...optionGrid,
    range: createLineRange(editor2.selection.anchor)
    // anchor range on editor based on current selection
  };
  ...
  selectionContentText[multiLineAnchorText].contentText.forEach(
    contentTextFuncBuffered(
      // callback attach to render decorationRenderObject 
      editor2.setDecorations,
      // on pre-created decorationType that already are in registry
...
var asendingByLineNo = (a, b) => a.end.line - b.end.line;
var multiCursorIndexBuffer = [];
var multiCursorSelection = (editor2, previousKey) => {
  clearBufferOfhexkey(editor2.setDecorations, previousKey);
  const selectionText = selectionContentText[multiCursorText].contentText;
  const sorted = [...editor2.selections].sort(asendingByLineNo);
  for (let idx = 0; idx < sorted.length; idx++) {
    ...
    const selectionTextInfo = selectionText.map((decorationOption2) => vscode3.window.createTextEditorDecorationType(decorationOption2));
    multiCursorIndexBuffer.push(idx + 1);
    context2.idx = multiCursorIndexBuffer.join(",");
