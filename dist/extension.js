...
var isWholeLine = /isWholeLine/s;
var beforeCursor = /beforeCursor/s;
var afterCursor = /afterCursor/s;
var atLineStart = /atLineStart/s;
var selectionOnly = /selectionOnly/s;

// src/numeric/binary.ts
...
var allOccurrence = 64 /* ALL_OCCURRENCE */;
var nextOccurrenceInit = 16 /* NEXT_OCCURRENCE_INIT */;
var nextOccurrence = 32 /* NEXT_OCCURRENCE */;
...
var SELECTION_KIND_LIST_EXCLUDE_MULTI_CURSOR = [
  cursorOnly,
  singleLine,
  multiLine
];
...
var parseDiagnostic = (state, severity, fsPath, activeEditorfsPath) => {
...
var updateDiagnostic = (activeEditorUri) => {
...
var columnsOfCol = 0;
var columnsOfZCol = 0;
var lineNumberOfLn = 0;
var characterCountOfChar = 0;
var multiCursorLineCol = 0;
var multiCursorLineZCol = 0;
var multiCursorLineNth = 0;
var multiCursorLineNumber = 0;
var multiCursorLineCount = 0;
var multiCursorLineCountFull = 0;
var multiCursorLineCharacter = 0;
...
var replicateNthRenderOption = (contentText, state, context2) => {
  ...
  state.overlay.indexListRefBuffer.push(context2.indexList);
...
var multiCursorIndexControl = (state, context2) => {
  ...
  if (state.duplicateOverlayIndex !== void 0) {
    ...
    const fnStep = duplicateOeverlayFunc[signature2];
...
var columns = {
  col: (editor2) => columnDelta(editor2, 1),
  zCol: (editor2) => columnDelta(editor2)
};
var characterCount = {
  char: (editor2) => {
...
var characterCountOf = {
  char: characterCountOfChar
};
var multiCursorOf = {
  col: multiCursorLineCol,
  zCol: multiCursorLineZCol,
  count: multiCursorLineCountFull,
  nth: multiCursorLineNth,
  ln: multiCursorLineNumber,
  lc: multiCursorLineCount,
  char: multiCursorLineCharacter
};
var selectionOf = {
  ["cursorOnlyText" /* CURSOR_ONLY_TEXT */]: { ...columnsOf, ...lineNumberOf },
  ["singleLineText" /* SINGLE_LINE_TEXT */]: { ...lineNumberOf, ...characterCountOf },
  ["multiLineCursorText" /* MULTI_LINE_CURSOR_TEXT */]: { ...lineNumberOf, ...multiLineOf },
  ["multiLineAnchorText" /* MULTI_LINE_ANCHOR_TEXT */]: { ...lineNumberOf, ...multiLineOf },
...
var setMultiCursorContentRef = () => {
  const multiCursorTextRef2 = selectionContentText[multiCursorText];
  if (multiCursorTextRef2 !== void 0) {
    textContext.contentText = multiCursorTextRef2.contentText;
  }
  const multiCursorEditRef2 = selectionContentText[multiCursorEdit];
  if (multiCursorEditRef2 !== void 0) {
    editContext.contentText = multiCursorEditRef2.contentText;
  }
};
var setContextAccmulator = () => {
...
var cursorOnlySelection = (editor2, previousCursor) => {
  ...
  selectionStatusFunctionChain[cursorOnlyText].forEach(functionChain(cursorOnlyStatusRef, editor2));
...
var singleLineSelection = (editor2, previousCursor) => {
  ...
  selectionStatusFunctionChain[singleLineText].forEach(functionChain(singleLinetatusRef, editor2));
...
var getContextKind = (ifEditMode) => {
  return ifEditMode ? editContext : textContext;
};
var getMultiCursorKind = (ifEditMode) => {
  return ifEditMode ? multiCursorText + editBitPosition : multiCursorText;
};
...
var cursorMalformationGuard = async (editor2) => {
  if (editor2.selection.isEmpty !== editor2.selections[editor2.selections.length - 1].isEmpty) {
    editor2.selections = await [...editor2.selections.slice(0, editor2.selections.length - 1)];
    return true;
  }
  return false;
};
var callFnChain = (state, context2) => (fn) => fn(state, context2);
var multiCursorSelection = async (editor2, previousCursor) => {
  await cursorMalformationGuard(editor2);
  multiCursorState.strategyKey === 0 && await clearSelectionTextBuffer(editor2, SELECTION_KIND_LIST_EXCLUDE_MULTI_CURSOR);
...
var clearSelectionTextBuffer = (editor2, selectionList = SELECTION_KIND_LIST) => {
  for (const cursorType of selectionList) {
    clearBufferOfhexkey([cursorType], editor2.setDecorations);
  }
};
...
var convertToDecorationRenderOption = (config, isWholeLine2 = true, contentText = void 0) => {
  const decorationOption = { ...DECORATION_OPTION_CONFIG };
  decorationOption.isWholeLine = isWholeLine2;
...
var borderPositionParser = (selectionType, borderPosition2) => {
  ...
  let isWholeLine2 = false;
  let beforeCursor2 = false;
  let afterCursor2 = false;
  let atLineStart2 = false;
  let selectionOnly2 = false;
  if (position.length > 1) {
    isWholeLine2 = isWholeLine.test(position[1]);
    beforeCursor2 = beforeCursor.test(position[1]);
    afterCursor2 = afterCursor.test(position[1]);
    atLineStart2 = atLineStart.test(position[1]);
    selectionOnly2 = selectionOnly.test(position[1]);
    if (selectionType === multiLine && position[0] === "left") {
      isWholeLine2 = true;
    }
  }
  return {
    isWholeLine: isWholeLine2,
    borderPosition: position[0],
    beforeCursor: beforeCursor2,
    afterCursor: afterCursor2,
    atLineStart: atLineStart2,
    selectionOnly: selectionOnly2
  };
};
...
function activate(context2) {
  ...
  return context2.extension.activate();
}
function deactivate() {
}
